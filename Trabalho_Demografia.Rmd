...
mortalidade_feminina <- mortalidade %>% 
  mutate(ano_obt=str_sub(DTOBITO, 5,8),
         data_obito = lubridate::dmy(DTOBITO),
         data_nasc = lubridate::dmy(DTNASC),
         idade_dias = as.numeric(data_obito-data_nasc) ,
  ...
  filter(ano_obt %in% c("2010", "2019", "2021"),
         sexo=="F") %>% 
  rename(Ano=ano_obt) %>% 
  mutate(faixa=cut(idade, breaks=c(-1, 1, 5, 10, 15, 20,
                                   25, 30, 35, 40, 45, 50, 
                   ...
                   labels=c("<1", "1-4", "5-9", "10-14", "15-19",
                            ...
                            "60-64", "65-69", "70-74", "75-79", 
                            "80-84", "85-89", ">90"), right=F)) %>% 
  filter(faixa %in% c("25-29", "30-34", "35-39", 
                            "40-44", "45-49")) %>% 
  mutate(LI=as.numeric(str_sub(faixa, 1,2)),
         tempo_intervalo=idade-LI) %>% 
  #select(faixa, LI, idade,  tempo_intervalo) %>% 
  group_by(Ano, faixa) %>% 
  summarize(obitos=n(),
              kx=mean(tempo_intervalo)) %>% left_join(pop_fem_fertil_faixa) %>% 
  mutate(mort=obitos/Pop*1000) %>% 
  mutate(lx=c(1000, rep(NA,4))) %>% 
  mutate(faixa=as.character(faixa))

morts <-list()
lo <- list("2010"= c(1000, rep(NA,4)),
             "2019"= c(1000, rep(NA,4)),
                       "2021"= c(1000, rep(NA,4)))

for(j in unique(mortalidade_feminina$Ano)){
morts[[j]] <- mortalidade_feminina %>% 
    filter(Ano==j) %>% 
    pull(mort)

for (i in 1:(length(morts[[j]])-1)){
  lo[[j]][i+1] <-  lo[[j]][1]-morts[[j]][i] 
}
}
  
lx <- data_frame("2010"=lo[["2010"]],
                 "2019"=lo[["2019"]],
                 "2021"=lo[["2021"]],
                 faixa=c("25-29", "30-34", "35-39", 
                            "40-44", "45-49"))%>% 
  pivot_longer(-faixa, names_to = "Ano",
               values_to = "lx") 

left_join(mortalidade_feminina, lx)
