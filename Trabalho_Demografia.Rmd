...
# read.dbc eh um pacote especifico para importacao de dados do datasus
# ref: https://docs.bvsalud.org/biblioref/2018/07/906543/anais_cbis_2016_artigos_completos-601-606.pdf
...
#concatenando strings em uma única para formar o nome e juntar em uma lista

dados_list <- map(arquivos_nome,~ read.dbc(.x))
#funcao MAP remove a necessidade de declararmos um objeto iterador auxiliar (i) e aplica a funcao desejada em cada elemento do objeto dado
...
titulo <-"Tabela 1: Probabilidade de sobrevivência de um recém-nascido à idade exata 5 para as coortes de 2000 a 2016 em Santa Catarina"

cbind(lo$Ano_nascimento, npx$`sum(n)`) %>% 
  kable(format="html", caption=titulo, booktabs = T, digits=4, linesep="", align=c("c","c"),col.names=c("Ano","Probabilidade")) %>% 
...
titulo2 <-"Tabela 2: Probabilidade de sobrevivência ao primeiro aniversário dos recém-nascidos em Santa Catarina no período de 2000 a 2020"

cbind(lo$Ano_nascimento, npx0$`sum(n)`) %>% 
  kable(format="html", caption=titulo2, booktabs = T, digits=4, linesep="", align=c("c","c"),col.names=c("Ano","Probabilidade")) %>% 
  kable_styling(latex_options = "hold_position",position="center")
```
...
Tendo como comparativo o método apresentado para a taxa de mortalidade infantil, junto ao fator que calcula a probabilidade de sobrevivência do nascido até o mesmo completar um ano de vida, observa-se que o segundo indicador trabalha com valores absolutos, apresentados diretamente, entretanto a taxa de mortalidade infantil, se caracteriza pela média móvel dos nascimentos, com o objetivo de diminuir o impacto de um ano que possa ter um comportamento fora do normal, por acontecimentos naturais ou algum problema de coleta e seleção.
Para as taxas calculadas pela pesquisa relacionada à mortalidade de menores de 1 ano do RIPSA, os dados apresentam queda nesse fato com o decorrer dos anos.
Embora possa se observar essa tendencia, temos que as taxas obtidas a partir dos dados do SIM e do SINASC são menores que as apresentadas na última pesquisa do RIPSA. Mesmo que a diferença apresentada seja baixa, nunca maior do que 1 ponto percentual, um fator explicativo, pode ser a diferença entre datas, já que, as taxas obtidas no item b são de 2021 e a última taxa calculada pelo RIPSA é de 2011.
