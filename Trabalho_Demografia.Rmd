...
Abaixo (Figura \ref{fig:fig1}) podemos observar o gráfico dessa dinâmica. No ano de 2010 foi mais comum a ocorrência de mães adolescentes. Nos anos de 2019 e 2021 houve um aumento na reprodução no estado, com aumento nos nascimentos para todas as outras faixas-estárias maternas. 
...
A taxa bruta de repodução poderia ser equivalente a taxa de fecundidade total, mas dessa vez considerando apenas a população feminina. Na Tabela \@ref(tab:tbr) abaixo podemos observar esses valores. 
...
Finalmente, a taxa líquida de reprodução leva em consideração o risco de mortalidade em cada faixa-etária. Notamos então (Tabela \@ref(tab:tlr)) valores líquidos ligeiramentes menores do que o apresentado na Tabela \@ref(tab:tbr) anterior. 
```{r tlr}
...
Para permitir uma análise mais direta, a variável de idade foi dicotomizada em mulheres com mais ou menos de 30 anos. Já a variável de escolaridade foi dicotomizada em mulheres que possuem ou não ao menos o Ensino Fundamental (ESCMAE>=4, que corresponde a pelo menos 8 anos de estudo, que por sua vez corresponde a, pelo menos o Ensino Fundamental completo). Observações com valores faltantes foram excluídas. Nesse caso, notamos que a proporção de mulheres com ensino fundamental é a mesma independente da idade (*ca.* 70%). O teste de $\chi^2$ apresentou um p-valor de `r chi1`, indicando que não há evidência de associação quando as variáveis são agrupadas dessa forma. Isso também pode ser observado pelo *Odds Ratio* dessa relação é de `r odd1`, indicando que as chances nos dois grupos é muito próxima. Abaixo podemos visualizar essa relação no gráfico de mosaico (Figura \ref{fig:mosaico1}). Apesar de haver numericamente mais mulheres que possuem Ensino Fundamental na base de dados, a proporção de mulheres em cada faiza é bem semelhante no grupo sem Ensino Fundamental.
...
Tanto na tábua feminina quanto na masculina,  é possível observar que os fatores de separação nkx a partir da idade x = 5 são próximos de n2 = 2,5. Ou seja, admitindo que a sobrevivência lx varia de forma linear nesses grupos, a aproximaçãoo chegaria em resultados satisfatórios, próoximos do encontrado usando os microdados. O fator de separação no primeiro grupo etário 1k0 = 0,1220 para mulheres e 1k0 = 0,1160 para os homens indica que os óbitos em menores de 1 ano em Santa Catarina, 2021, ocorrem em maioria no início do intervalo etário, entre o 1o e 2o mês de vida. Esse resultado está em consonância com aqueles encontrados em países desenvolvidos.
...
No modelo, lx representa o número de pessoas que alcançam com vida a idade exata x de uma uma geração inicial de l0 nascimentos e, portanto,  é uma função de sobrevivência que inicia num determinado momento, com toda a população ainda viva, e permite calcular a porcentagem dessa população ainda viva em qualquer outro momento ao longo do tempo. Assim, l0 = 1 indica a geração inicial de nascimentos, que neste caso  é 1, e l20 e l60 seria o número de pessoas que alcançam com vida a idade exata 20 e 60, respectivamente. Portanto, esses valores são as probabilidades de sobreviver a idade exata 20 e 60.
Por outro lado, nqx  é a probabilidade que uma pessoa, tendo alcançado a idade exata x, tem de morrer entre as idades x e x + n. Nota-se que as probabilidades de morte para as primeiras faixas etárias são menores do que para as últimas, já que a sobreviência seria maior entre as pessoas mais jovens. No entanto, a mortalidade infantil ainda apresenta valores expressivos, como pode ser observado no gráfico.

# Anexo

O código desta análise pode ser acessado no [GitHub](https://github.com/cmusso86/Trabalho_Demografia/blob/master/Trabalho_Demografia.Rmd).
